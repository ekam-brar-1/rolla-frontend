...
export default function CustomizeShirt() {
  ...
  const handleUpload = async () => {
    if (!frontDesign.image || !backDesign.image) {
        alert("Both front and back designs are required!");
        return;
    }
...
const handleImageUpload = (e, isFront = true) => {
  ...
  return (
    <div className="App"  style={{ zIndex: 1 }} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
      ...
      {step === 1 && (
        <div className="text-center">
          ...
          <div className="d-flex flex-wrap justify-content-center mt-3">
            {["#ffffff", "#000000", "#FF5733", "#33FF57", "#3357FF"].map((color) => (
              <button
                key={color}
                style={{ backgroundColor: color, width: 50, height: 50, margin: 5 }}
                onClick={() => setShirtColor(color)}
                className={`btn ${shirtColor === color ? "border border-dark" : ""}`}
              />
            ))}
          </div>
          <button className="btn btn-primary mt-3" onClick={handleNextStep} disabled={!shirtColor}>
            Next Step
          </button>
        </div>
      )}
        ...
        <div className="text-center">
          ...
          <div className="d-flex flex-wrap justify-content-center mt-3">
            {["XS", "S", "M", "L", "XL", "XXL"].map((size) => (
              <button
                key={size}
                className={`btn btn-outline-primary mx-2 ${shirtSize === size ? "active" : ""}`}
          ...
          <div className="mt-3">
            <button className="btn btn-secondary me-3" onClick={handlePreviousStep}>
              Previous Step
            </button>
            <button className="btn btn-primary" onClick={handleNextStep} disabled={!shirtSize}>
              Next Step
            </button>
          </div>
        </div>
...
/>
          ...
          <div className="mt-3">
            ...
            <button className="btn btn-success" onClick={()=>{handleAddToCart();handleUpload()}}>
              Add to Cart
            </button>
          </div>
        </div>
